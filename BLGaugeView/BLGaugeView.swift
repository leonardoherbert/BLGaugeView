...
open class BLGaugeView: UIView {
    ...
    private let percentLabelValues = [0, 0.375, 0.625, 1]
   ...
   private func setup() {
    ...
    private func drawBackground() {
        ...
        let endRedDegree    = startRedDegree + CGFloat(0.5625 * Double.pi)

        let startYellowDegree  = endRedDegree
        let endYellowDegree    = startYellowDegree + CGFloat(0.375 * Double.pi)

        let startGreenDegree   = endYellowDegree
        let endGreenDegree     = endYellowDegree + CGFloat(0.5625 * Double.pi)
